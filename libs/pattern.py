class Pattern(object):
    ...
    def matching(self, text):
        ...
        for line in text:
            formatedTxt += ' ' + line
        textMatched, textArray, tmp = [[], (formatedTxt.strip()).split(' '), '']
        for word in textArray:
            tmp += ' ' + (word.strip()).lower()
            for wd in textMatched:
                tmp = (tmp.replace(wd, '')).strip()
            if tmp.strip() in self.kb:
                if tmp.strip() not in textMatched:
                    textMatched.append(tmp.strip())
        return textMatched
    ...
    def toWav(self, name):
        librosa.output.write_wav(name, self.y, self.sr)

    def vad(self):
        v = VoiceActivityDetector(self.src)
        raw_detection = v.detect_speech()
        speech_labels = v.convert_windows_to_readible_labels(raw_detection)
        return speech_labels

    def writeWav(self, name, src):
        D = librosa.stft(src, n_fft=2048, hop_length=512)
        D_sp = librosa.phase_vocoder(D, 1, hop_length=512)
        y_sp = librosa.istft(D_sp, hop_length=512)
